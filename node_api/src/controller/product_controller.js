...
exports.addProduct = async (req, res) => {
    ...
    if ( typeof price !== "number")
        return res.status(400).json({ message: 'Invalid price! Please send numerical value' });
    if (typeof quantity !== "number")
        return res.status(400).json({ message: 'Invalid quantity! Please send numerical value' });
    let files = req.files.map(file => { return `http://localhost/public/uploads/${file.filename}` });
